...
<label for="menu-toggle" id="hamburger">
  ...
  <span></span>
      ...
      <h1 class="my-courses">Archived Student</h1>
      <div class="filter">
        <label for="filter" class="my-courses">Filter Courses:</label>
        <form action="archiveStudent.php" method="POST">
    ...
    <table>
      ...
      <tbody>
        ...
        if(isset($_POST['submit'])){
          $data = selectData("student", "*", "status = 0 AND course_id ='".$_POST['filter']."'");
        }else{
        $data = selectData("student", "*", "status = 0");
        }
        if (!empty($data)) {
          $i = 1;
          foreach ($data as $row) {
...
<div class="table-content">
  ...
  <tbody>
    <?php
  $data = selectData("student", "*", " status=0 AND student_id = '" . $_POST['student_id'] . "'");
  if (!empty($data)) {
    $i = 1;
    foreach ($data as $row) {
      $course_info = selectData("course", "*", "course_id='" . $row['course_id'] . "'");
