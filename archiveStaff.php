...
<label for="menu-toggle" id="hamburger">
  ...
  <span></span>
      ...
      <h1 class="my-courses">Archived Staff</h1>
      <div class="current-year" class="my-courses">2022/2023</div>
    </header>
<div class="search-container">
  <form action="archiveStaff.php" method="POST" style="margin-bottom:4px;">
...
<?php if(!isset($_POST['search'])){?>
  ...
  <tbody>
    <?php
  $data = selectData("staff","*","status = 0");
if (!empty($data)) {
     $i = 1;
    foreach ($data as $row) {
    ?> <tr>
      ...
      <td><a href="unarchiveStaf.php?id=<?php echo $row['staff_id']?>"><i class="fa fa-archive"></i></a></td>
...
<div class="table-content">
  ...
  <tbody>
    <?php
  $data = selectData("staff", "*", " staff_id = " . $_POST['staff_id']." AND status=0" );
if (!empty($data)) {
     $i = 1;
    foreach ($data as $row) {
    ?> <tr>
